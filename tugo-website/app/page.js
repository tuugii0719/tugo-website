...
const sliderTours = [
  { slug: "terelj-escape",     title: "Terelj Escape",            emoji: "🌲", dates: "Jul 16 – 18", days: 3,  price: "$270",   img: "/images/destinations/terelj-park.jpg",        accent: "text-teal-300",    includes: "Transport · Meals · Ger · Guide", inactive: true },
                ...
                {sliderTours.map((t, i) => {
                  ...
                  return (
                    <Link
                      key={`${t.slug}-${i}`}
                      href={`/tours/${t.slug}`}
                      className={`snap-start flex-shrink-0 w-[240px] md:w-[260px] rounded-[24px] overflow-hidden bg-night-900/60 border border-sand-800/40 hover:border-sand-600/60 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.6)] transition-all group ${(t.again2027 || t.inactive) ? "opacity-75 hover:opacity-100" : ""}`}
                    >
                      <div className="relative aspect-[4/5]">
                        <Image
                          src={t.img}
                          alt={t.title}
                          fill
                          className={`object-cover transition-transform duration-700 group-hover:scale-[1.04] ${(t.again2027 || t.inactive) ? "grayscale opacity-40" : ""}`}
                          sizes="260px"
                        />
                        {t.limited && (
                        ...
                        {t.inactive && (
                          <span className="absolute top-3 left-3 inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-night-950/70 backdrop-blur-md border border-sand-600/40 text-sand-300 text-[9px] tracking-[0.2em] uppercase font-medium">
                            <span aria-hidden="true">—</span> Not in 2026
                          </span>
                        )}
                      </div>
                      <div className="p-4 md:p-5">
                        <p className={`${(t.again2027 || t.inactive) ? "text-sand-500" : t.accent} text-[10px] tracking-[0.2em] uppercase mb-1.5 font-medium`}>
                          {t.dates}
                        </p>
